...
const autopush = require('../lib/cjs')


var fixtures = path.join(__dirname, '/statics')
...
describe('autopush()', function () {
  describe('basic operations', function () {
    ...
    it('should require root path', function () {
      assert.throws(autopush.bind(null), /root path required/)
    })

    it('should require root path to be string', function () {
      assert.throws(autopush.bind(null, 42), /root path.*string/)
    })
  ...
  describe('fallthrough', function () {
    ...
    describe('when true', function () {
      ...
      it('should fall-through when URL too long', function (done) {
        ...
        app.use(autopush(root, { 'fallthrough': true }))
    ...
    describe('when false', function () {
      ...
      // this responds not as below, but sends a 404 with html error
      it.skip('should 404 when URL too long', function (done) {
        var app = http2Express(express)
        var root = fixtures + Array(10000).join('/foobar')

        app.use(autopush(root, { 'fallthrough': false }))
        app.use(function (req, res, next) {
          res.sendStatus(404)
        })
  ...
  describe('setHeaders', function () {
    ...
    beforeAll(function () {
      app = http2Express(express)
      app.use(autopush(fixtures, { 'setHeaders': function (res) {
        res.setHeader('x-custom', 'set')
      } }))
    })

    it('should reject non-functions', function () {
      assert.throws(autopush.bind(null, fixtures, { 'setHeaders': 3 }), /setHeaders.*function/)
    })
  ...
  describe('when index at mount point', function () {
    ...
    beforeAll(function () {
      app = http2Express(express)
      app.use('/users', autopush(fixtures + '/users'))
    })
  ...
  describe('when mounted', function () {
    ...
    beforeAll(function () {
      app = http2Express(express)
      app.use('/static', autopush(fixtures))
    })
  ...
  describe('when mounted "root" as a file', function () {
    ...
    beforeAll(function () {
      app = http2Express(express)
      app.use('/todo.txt', autopush(fixtures + '/todo.txt'))
    })
  ...
  describe('when index file serving disabled', function () {
    ...
    beforeAll(function () {
      app = http2Express(express)
      app.use('/static', autopush(fixtures, { 'index': false }))
      app.use(function (req, res, next) {
        res.sendStatus(404)
      })
    })
...
function createApp (dir, options) {

  const app = http2Express(express)
  const root = dir || fixtures

  app.use(autopush(root, options))
